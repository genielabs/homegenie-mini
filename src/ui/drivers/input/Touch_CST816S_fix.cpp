...
//----------------------------------------------------------------------------
  bool Touch_CST816S_fix::init(void)
  {
    _inited = false;
  ...
  size_t Touch_CST816S_fix::_read_data(uint8_t* readdata)
  {
    size_t res = 0;
  ...
  uint_fast8_t Touch_CST816S_fix::getTouchRaw(touch_point_t *tp, uint_fast8_t count)
  {
    ...
    do
    {
      ...
      count = readdata[0];
    } while ((count > 1 || (readdata[5] != 0x10 && readdata[5] != 0)) && --retry);
    if (!retry)
    {
      count = 0;
    }
