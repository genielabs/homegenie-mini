...
typedef void(*callback_t)(uint8_t num, Events_t event , String topic_name, uint8_t * payload, uint16_t length_payload);
...
class MQTTbroker_lite {
    ...
    private:
        WebSocketsServer * WS;
        callback_t callback;
        void runCallback(uint8_t num, Events_t event , uint8_t * topic_name = NULL, uint16_t length_topic_name = 0,uint8_t * payload = NULL, uint16_t length_payload = 0);
        void sendAnswer(uint8_t num, uint8_t fixed_header_comm, uint8_t fixed_header_lsb = 0, uint8_t fixed_header_remaining_length = 0, uint8_t * variable_header = NULL, uint8_t variable_header_length = 0, uint8_t * payload = NULL, uint16_t payload_length = 0);
        void sendMessage(uint8_t num, uint8_t * topic_name, uint16_t length_topic_name, uint8_t * payload, uint16_t length_payload);
